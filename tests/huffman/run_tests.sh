#!/bin/bash
# ***************************************************************************
#                      Huffman Compression Test Suite
#
...
HUFFMAN_BIN="$SCRIPT_DIR/../../bin/huffman"
...
# Check if huffman binary exists
if [ ! -f "$HUFFMAN_BIN" ]; then
    echo -e "${RED}✗ Error: Huffman binary not found at $HUFFMAN_BIN${NC}"
...
rm -f "$OUTPUT_DIR"/*.huf "$OUTPUT_DIR"/*.out
...
test_file() {
    ...
    local compressed="$OUTPUT_DIR/${basename}.huf"
    local decompressed="$OUTPUT_DIR/${basename}.out"
    
    printf "%-40s " "  $test_name"
    
    # Compress
    if ! "$HUFFMAN_BIN" -c "$input_file" "$compressed" > /dev/null 2>&1; then
    ...
    # Decompress
    if ! "$HUFFMAN_BIN" -d "$compressed" "$decompressed" > /dev/null 2>&1; then
